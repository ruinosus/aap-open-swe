...
async def get_agent(config: RunnableConfig) -> Pregel:  # noqa: PLR0915
    ...
    # Load system prompt from AAP manifest if available, fallback to construct_system_prompt
    ...
    # Load model config from AAP manifest, fallback to defaults
    model_id = get_model_id()
    model_temperature = get_model_temperature()
    model_max_tokens = get_model_max_tokens()

    logger.info(
        "Returning agent for thread %s (model=%s, manifest=%s)",
        thread_id,
        model_id,
        "yes" if manifest_instruction else "no",
    )
    return create_deep_agent(
        model=make_model(model_id, temperature=model_temperature, max_tokens=model_max_tokens),

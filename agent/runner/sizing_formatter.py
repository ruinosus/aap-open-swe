...
from agent.config import get_formatting, get_manifest
...
def format_sizing_markdown(agent_response: str) -> str:
    ...
    template_data = _build_template_data(
        data, layer_icons, severity_icons, status_icons, module_name
    )
...
def _build_template_data(
    ...
    return {
        ...
        "repo_type_label": (
            "\U0001f500 External (fork required)"
            if repo_type == "external"
            else "\U0001f3e0 Internal"
        ),
